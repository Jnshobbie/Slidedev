...
import { chatLimiter, rateLimitResponse } from '@/lib/ide-rate-limit';
...
export async function POST(req: NextRequest) {
  ...
  const { allowed, retryAfter } = chatLimiter(userId);
  ...
  if (!Array.isArray(files) || files.length > 20) {
    return NextResponse.json({ error: 'Too many files' }, { status: 400, headers: CORS_HEADERS });
  }
